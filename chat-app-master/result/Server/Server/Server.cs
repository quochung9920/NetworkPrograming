    ...
    public partial class Server : Form
    {
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive) // Kiểm tra xem luồng lắng nghe đã kết nối hay chưa
            {
                ...
                ip = Dns.Resolve(addrTextBox.Text).AddressList[0];
                //Log(SystemMsg(ip.ToString()));

                int port = -1;
                if (!int.TryParse(portTextBox.Text, out port)) // Chuyển đổi number từ chuỗi thành số nguyên và gán vào biến port
                {
