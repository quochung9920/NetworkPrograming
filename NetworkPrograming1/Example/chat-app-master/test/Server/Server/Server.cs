            ...
            if (bytes > 0)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        Dictionary<string, byte[]> data = json.Deserialize<Dictionary<string, byte[]>>(obj.data.ToString());
                        using (Bob bob = new Bob())
                        {
                            using (RSACryptoServiceProvider rsaKey = new RSACryptoServiceProvider())
                            {
                                bob.Receive(data["iv"], data["encryptedSessionKey"], data["encryptedMessage"]);
                            }
                        }
